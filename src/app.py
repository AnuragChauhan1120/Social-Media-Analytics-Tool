...
with st.sidebar:
    ...
    model_choice = st.sidebar.selectbox("Sentiment model", options=["TextBlob (default)", "VADER", "Transformer (RoBERTa)"], index=0)
...
if fetch_btn:
    ...
    try:
        ...
        elif model_choice == "Transformer (RoBERTa)":
            try:
                from transformer_models import TransformerSentimentModel
                t_sent = TransformerSentimentModel()
                texts = df["text"].astype(str).tolist()
                sent_res = t_sent.predict(texts, batch_size=32)

                df["transformer_sentiment"] = [r.get("sentiment_label", "neutral") for r in sent_res]
                df["t_positive"] = [r.get("positive_score", 0.0) for r in sent_res]
                df["t_negative"] = [r.get("negative_score", 0.0) for r in sent_res]
                df["t_neutral"]  = [r.get("neutral_score", 0.0) for r in sent_res]

                df["sentiment"] = df["transformer_sentiment"]
                df["sentiment_score"] = df["t_positive"] - df["t_negative"]
            except Exception as e:
                status.warning(f"Transformer sentiment failed: {e}")
                # keep any previous sentiment columns if present
    except Exception as e:
    ...
    if use_emotions:
        ...
        # Transformer emotions for RoBERTa
        elif model_choice == "Transformer (RoBERTa)":
            try:
                from cpu_emotion_model import CPUEmotionModel
                emo = CPUEmotionModel()
                texts = df["text"].astype(str).tolist()
                emo_res = emo.predict(texts, batch_size=16)
                emo_df = pd.DataFrame(emo_res)

                # remove NRC if present
                for col in nrc_cols:
                ...
                if not emo_df.empty:
                    df["dominant_emotion"] = emo_df.idxmax(axis=1)
                emo_added = True
                status.info("Transformer emotions added (CPU).")
            except Exception as e:
                status.warning(f"Transformer emotion model failed ({e}), falling back to NRC if available.")
                try:
                    from emotion_analysis import add_emotion_columns
                    if not set(nrc_cols).intersection(df.columns):
                        df = add_emotion_columns(df)
                    emo_added = bool(set(nrc_cols).intersection(df.columns))
                    if emo_added:
                        status.info("NRC emotions added as fallback.")
                except Exception as e2:
                    status.warning(f"NRC fallback also failed: {e2}")
